...
## SEO/GEO
- Desktop console = dock-console (Electron) at C:\开发\dockdocs文件\周边\dock-console (DockDocs 家族 2026-07-10 归拢到 dockdocs文件\周边\). As of 2026-06-15 it collects metrics LOCALLY (collectors.js: health/GEO-JSON-LD/internal-links/new-pages/external-mentions by fetching dockdocs.app directly). No server dependency. Its data/config lives at C:\开发\dockdocs文件\周边\dock-console-data\.
...
- GSC query data is LIVE in the console: collectors.js pulls Search Console via a local service-account key at C:\开发\dockdocs文件\周边\dock-console-data\dock-gsc-key.json (wired 2026-06-15, sc-domain resource). It shows "待接入" only if that key is missing.
