...
### 8.1 繁体中文 zh-Hant（派生语言）
- **不是从头翻译**，是简体的脚本转换：用 **opencc-js（cn→tw）** 在访问层 / `pick`·`tr` 的 `zh-Hant` 分支对 zh 值即时转繁（见 `lib/zh-hant.ts`）。改 helper、不改每个字符串。
- 切换器标签：zh = **「简体中文」**，zh-Hant = **「繁體中文」**。
- 首发 **noindex**，台/港用字母语审校通过后再放索引。OpenCC 偶有专有名词 over-convert，审校重点查这个。

### 8.2 德语 de（决策已定：用 **Sie** 正式称谓）
- **称谓 = Sie（正式）**〔已拍板〕——全站统一,禁止混用 du。这是德语唯一的前置品牌决策。
- **德语要真翻译**(非派生),流程同 es/pt/fr/ja;de 已在 `allLocales`(标签 "Deutsch"),只差内容 + 路由。
- **德语三大专属功夫(比别的语言多做)**:
  1. **文本膨胀 + 长复合词 → 溢出审计(头号)**:德语比英文长 20–35%,且复合词无空格不断行。按**渲染结果**查定宽 UI(nav/按钮/plan badge/tabs/表头/eyebrow/pill);加 `hyphens:auto`(配 `lang="de"`)+ `overflow-wrap:anywhere`,定宽 pill 改弹性宽。
  2. **整句翻译、禁止拼片段**:`pick/tr` 每条整串(已是);逐条审 `${n} Seite(n)` 这类插值句的复数+变格。
  3. **母语审 + 视觉溢出审 双过才摘 noindex**:德语 MT 常错在称谓一致性、复合词断字、名词大写、SEO 标题超长。
- 格式:数字 `1.000,00`、日期 `DD.MM.YYYY`、引号 `„…"`、`ß`/umlaut;SEO 标题/描述更长,`≤60/≤160` 卡更紧;关键词 `komprimieren/zusammenfügen/umwandeln/bearbeiten`。
