...
type Props = { locale?: "en" | "zh" | "es" | "pt" | "fr" | "ja"; name: string; nameZh?: string; nameEs?: string; namePt?: string; nameFr?: string; nameJa?: string };
...
export function ComingSoonTool({ locale = "en", name, nameZh, nameEs, namePt, nameFr, nameJa }: Props) {
  ...
  const fr = locale === "fr";
  const ja = locale === "ja";
  const label = es ? (nameEs ?? name) : zh ? (nameZh ?? name) : pt ? (namePt ?? nameEs ?? name) : fr ? (nameFr ?? nameEs ?? name) : ja ? (nameJa ?? name) : name;
  return (
    <div className="mx-auto flex max-w-xl flex-col items-center px-5 py-28 text-center sm:py-36">
      ...
      <p className="mt-3 text-[15px] font-semibold text-[color:var(--accent-strong)]">{zh ? "即将推出" : es ? "Próximamente" : pt ? "Em breve" : fr ? "Bientôt disponible" : ja ? "近日公開" : "Coming soon"}</p>
      ...
      <a
        href={zh ? "/zh/" : es ? "/es/" : pt ? "/pt/" : fr ? "/fr/" : ja ? "/ja/" : "/"}
      ...
      >
        {zh ? "查看全部工具" : es ? "Ver todas las herramientas" : pt ? "Ver todas as ferramentas" : fr ? "Voir tous les outils" : ja ? "すべてのツールを見る" : "Browse all tools"}
      </a>
    </div>
  );
}
