...
const _en = {
  ...
  desktopNote: "PDF tools in the web app work offline; AI chat and format conversion require an internet connection. The desktop app adds OS-level file integration — your files never pass through a browser engine.",
  privacyHeading: "Privacy",
  privacyNote: "Most DockDocs tools process your files locally in your browser — nothing is uploaded to our servers. A few conversion tools use our secure cloud service; files are deleted from the conversion service right after your download completes. The desktop app takes this further: your files never touch a browser engine at all.",
...
const COPY = {
  ...
  zh: {
    ...
    windowsExt: ".exe 安装包",
    macExt: ".dmg 安装包",
    desktopNote: "网页应用中的 PDF 工具支持离线使用；AI 问答与格式转换需要联网。桌面客户端进一步提升：与操作系统深度集成，文件处理完全不经过浏览器引擎。",
    privacyHeading: "隐私",
    privacyNote: "DockDocs 大多数工具在浏览器中本地处理文件，不上传到服务器。少数转换工具使用安全的云服务，下载完成后即从转换服务中删除。桌面客户端更进一步：文件完全不经过浏览器引擎。",
    privacyLink: "隐私政策 →",
  },
  es: {
    ...
    desktopNote: "Las herramientas PDF de la app web funcionan sin conexión; el chat con IA y la conversión de formato necesitan internet. La app de escritorio añade integración a nivel del SO — los archivos nunca pasan por un motor de navegador.",
    privacyHeading: "Privacidad",
    privacyNote: "La mayoría de las herramientas de DockDocs procesan tus archivos localmente en el navegador — nada se sube a nuestros servidores. Algunas herramientas de conversión usan nuestro servicio en la nube seguro; los archivos se eliminan del servicio de conversión en cuanto termina tu descarga. La app de escritorio va más lejos: los archivos nunca pasan por un motor de navegador.",
  ...
  pt: {
    ...
    windowsExt: "instalador .exe",
    macExt: "instalador .dmg",
    desktopNote: "As ferramentas PDF do app web funcionam offline; o chat com IA e a conversão de formato precisam de internet. O app desktop adiciona integração ao nível do SO — seus arquivos nunca passam por um motor de navegador.",
    privacyHeading: "Privacidade",
    privacyNote: "A maioria das ferramentas do DockDocs processa seus arquivos localmente no navegador — nada é enviado a nossos servidores. Algumas ferramentas de conversão usam nosso serviço em nuvem seguro; os arquivos são excluídos do serviço de conversão assim que o download for concluído. O app desktop vai mais longe: seus arquivos nunca passam por um motor de navegador.",
  ...
  fr: {
    ...
    desktopNote: "Les outils PDF de l'app web fonctionnent hors ligne ; le chat IA et la conversion de format nécessitent une connexion internet. L'app bureau ajoute une intégration OS — vos fichiers ne passent jamais par un moteur de navigateur.",
    privacyHeading: "Confidentialité",
    privacyNote: "La plupart des outils DockDocs traitent vos fichiers localement dans votre navigateur — rien n'est envoyé à nos serveurs. Quelques outils de conversion utilisent notre service cloud sécurisé ; les fichiers sont supprimés du service de conversion dès que votre téléchargement est terminé. L'app bureau va plus loin : vos fichiers ne passent jamais par un moteur de navigateur.",
  ...
  ja: {
    ...
    macExt: ".dmg インストーラー",
    desktopNote: "ウェブアプリの PDF ツールはオフラインで動作します。AIチャットとフォーマット変換はインターネット接続が必要です。デスクトップアプリは OS レベルのファイル統合を追加 — ファイルはブラウザエンジンを一切介しません。",
    privacyHeading: "プライバシー",
    privacyNote: "DockDocs のほとんどのツールはファイルをブラウザ内でローカル処理 — サーバーへのアップロードはありません。一部の変換ツールは安全なクラウドサービスを使用します。ファイルはダウンロード完了後に変換サービスから削除されます。デスクトップアプリはさらに徹底：ファイルはブラウザエンジンに一切触れません。",
  ...
  de: {
    ...
    windowsExt: ".exe Installer",
    macExt: ".dmg Installer",
    desktopNote: "Die PDF-Tools der Web-App funktionieren offline; KI-Chat und Formatkonvertierung benötigen eine Internetverbindung. Die Desktop-App fügt OS-Integration hinzu — Ihre Dateien berühren keinen Browser-Engine.",
    privacyHeading: "Datenschutz",
    privacyNote: "Die meisten DockDocs-Tools verarbeiten Ihre Dateien lokal im Browser — nichts wird auf unsere Server hochgeladen. Einige Konvertierungstools nutzen unseren sicheren Cloud-Service; Dateien werden nach Ihrem Download vom Konvertierungsdienst gelöscht. Die Desktop-App geht weiter: Ihre Dateien berühren keinen Browser-Engine.",
  ...
  ko: {
    ...
    macExt: ".dmg 설치 파일",
    desktopNote: "웹 앱의 PDF 도구는 오프라인에서 작동합니다. AI 채팅과 형식 변환은 인터넷 연결이 필요합니다. 데스크톱 앱은 OS 수준의 파일 통합을 추가하며 파일이 브라우저 엔진을 거치지 않습니다.",
    privacyHeading: "개인정보 보호",
    privacyNote: "DockDocs의 대부분 도구는 브라우저에서 파일을 로컬로 처리합니다 — 서버에 업로드되지 않습니다. 일부 변환 도구는 안전한 클라우드 서비스를 사용하며 파일은 다운로드 완료 후 변환 서비스에서 삭제됩니다. 데스크톱 앱은 한층 더 나아가 파일이 브라우저 엔진에 전혀 닿지 않습니다.",
