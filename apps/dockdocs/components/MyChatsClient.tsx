...
const STR = {
  ...
  ja: {
    eyebrow: "マイチャット",
    heroTitle: "保存された「PDFと対話」履歴。",
    heroIntro: "サインイン済みのユーザーは、チャット履歴とアップロードしたドキュメントのメタデータを後で確認するために保存できます。元の PDF ファイルは保存されません。",
    loading: "アカウントを読み込み中…",
    signInTitle: "サインインしてワークスペースのデータを分離。",
    signInDesc: "DockDocs は匿名利用のためにブラウザのローカル記録を保持し、サインイン後はアカウント ID ごとにデータを分離できます。元の PDF は保存されません。",
    currentStorage: "現在のストレージ",
    plan: "プラン",
    ...
    browser: "ブラウザ",
    signedIn: "サインイン済み",
    workspaceStorage: "ワークスペースのストレージ",
    saved: "保存済み",
    clearChats: "アカウントのチャットを消去",
    noChatsTitle: "まだアカウントのチャットがありません。",
    noChatsDesc: "サインインした状態で「PDFと対話」を始めてください。DockDocs が質問、回答、出典、トークン使用量、ドキュメントのメタデータを保存します。",
    openChat: "「PDFと対話」を開く",
    turnsLabel: (n: number) => `${n} ターン`,
    user: "ユーザー",
    assistant: "アシスタント",
    context: "コンテキスト",
    trimmed: " · 切り詰め済み",
    provider: "プロバイダー",
    tokenUsage: "トークン使用量",
    totalTokens: (n: number) => `合計 ${n}`,
    notAvailable: "利用不可",
    workspace: "ワークスペース",
    account: "アカウント",
    local: "ローカル",
    todayChat: "本日の AI チャット",
    totalTokensLabel: "トークン合計",
    savedSessions: "保存済みセッション",
    resumeWork: "ドキュメント作業を再開",
    sessionsCount: (n: number) => `${n} セッション`,
    noSessions: "まだ保存されたセッションがありません。",
    sessionMeta: (turns: number, tokens: number) => `${turns} ターン · ${tokens} トークン`,
    restore: "復元",
    delete: "削除",
    references: "出典",
    verifiedBadge: "出典と照合済み",
    savedDocs: "保存済みドキュメント",
  ...
  ko: {
    eyebrow: "내 채팅",
    heroTitle: "저장된 'PDF와 대화' 기록.",
    heroIntro: "로그인한 사용자는 채팅 기록과 업로드한 문서 메타데이터를 나중에 확인하기 위해 저장할 수 있습니다. 원본 PDF 파일은 저장되지 않습니다.",
    loading: "계정 불러오는 중…",
    signInTitle: "워크스페이스 데이터를 분리하려면 로그인하세요.",
    signInDesc: "DockDocs는 익명 사용을 위해 브라우저에 로컬 기록을 보관하고, 로그인 후에는 계정 ID별로 데이터를 분리합니다. 원본 PDF는 저장되지 않습니다.",
    ...
    saved: "저장됨",
    clearChats: "계정 채팅 삭제",
    noChatsTitle: "아직 계정 채팅이 없습니다.",
    noChatsDesc: "로그인 상태에서 PDF와 대화를 시작하세요. DockDocs가 질문, 답변, 참조, 토큰 사용량, 문서 메타데이터를 저장합니다.",
    openChat: "PDF와 대화 열기",
    turnsLabel: (n: number) => `${n}턴`,
    user: "사용자",
    assistant: "어시스턴트",
    context: "컨텍스트",
    trimmed: " · 축약됨",
    ...
    resumeWork: "문서 작업 재개",
    sessionsCount: (n: number) => `세션 ${n}개`,
    noSessions: "아직 저장된 세션이 없습니다.",
    sessionMeta: (turns: number, tokens: number) => `${turns}턴 · ${tokens}토큰`,
    restore: "복원",
    delete: "삭제",
    references: "참조",
    verifiedBadge: "출처 확인됨",
    savedDocs: "저장된 문서",
    noDocs: "아직 저장된 문서 메타데이터가 없습니다.",
    docMeta: (chats: number, analyses: number) => `채팅 ${chats}건 · 분석 ${analyses}건`,
  },
};

export function MyChatsClient({ locale = "en" }: { locale?: Locale }) {
  // zh-Hant derives Traditional from zh via deepHant; all other locales now authored.
