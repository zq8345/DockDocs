  ...
  "batch-protect-pdf": {
    ...
    items: {
      en: [
        ...
        { q: "Are my files uploaded to a server?", a: "Yes. Encrypting a PDF requires our secure conversion server, so each file is uploaded, encrypted, returned, and then removed after processing — no account, and nothing is kept after your download. The password is sent only to set the encryption and isn't stored." },
        ...
        { q: "Are there rules for the password or limits on how many files?", a: "The password must be 4–32 characters using only letters, digits, and the underscore (_) — that keeps it safe to apply across every PDF reader. You can encrypt up to 30 files per batch; for more, just run the tool again. There's no hard size limit, but because each file is uploaded and encrypted on our server, very large batches take longer." },
      ...
      zh: [
        ...
        { q: "我的文件会被上传到服务器吗？", a: "会。加密 PDF 需要用到我们的安全转换服务器，所以每份文件都会被上传、加密、传回，处理后即删除——无需注册，下载后不留任何副本。密码仅用于设置加密、不会被存储。" },
        ...
        { q: "密码有规则吗？文件数量有上限吗？", a: "密码必须是 4–32 位，且只能用字母、数字和下划线(_)——这样才能保证在各种 PDF 阅读器里都通用。每批最多加密 30 份文件，要更多就再运行一次即可。没有硬性大小限制，但由于每份文件都要上传到服务器加密，批量特别大时会更慢一些。" },
      ...
      es: [
        ...
        { q: "¿Mis archivos se suben a un servidor?", a: "Sí. Cifrar un PDF requiere nuestro servidor de conversión seguro, así que cada archivo se sube, se cifra, se devuelve y se elimina tras el procesamiento; sin cuenta y sin conservar copia alguna tras tu descarga. La contraseña solo se envía para aplicar el cifrado y no se almacena." },
        ...
        { q: "¿Hay reglas para la contraseña o límites de cuántos archivos?", a: "La contraseña debe tener entre 4 y 32 caracteres usando solo letras, dígitos y el guion bajo (_); así es seguro aplicarla en cualquier lector de PDF. Puedes cifrar hasta 30 archivos por lote; para más, vuelve a ejecutar la herramienta. No hay un límite estricto de tamaño, pero como cada archivo se sube y se cifra en nuestro servidor, los lotes muy grandes tardan más." },
...
const FAQS_PT: Record<string, { title: string; items: Array<{ q: string; a: string }> }> = {
  ...
  "batch-protect-pdf": {
    ...
    items: [
      ...
      { q: "Meus arquivos são enviados a um servidor?", a: "Sim. Criptografar um PDF exige o nosso servidor de conversão seguro, então cada arquivo é enviado, criptografado, devolvido e removido após o processamento — sem conta e sem manter nenhuma cópia após o seu download. A senha é enviada apenas para aplicar a criptografia e não é armazenada." },
      ...
      { q: "Há regras para a senha ou limites de quantos arquivos?", a: "A senha deve ter entre 4 e 32 caracteres usando apenas letras, dígitos e o sublinhado (_) — isso a torna segura para aplicar em qualquer leitor de PDF. Você pode criptografar até 30 arquivos por lote; para mais, execute a ferramenta novamente. Não há limite rígido de tamanho, mas como cada arquivo é enviado e criptografado no nosso servidor, lotes muito grandes demoram mais." },
...
const FAQS_FR: Record<string, { title: string; items: Array<{ q: string; a: string }> }> = {
  ...
  "batch-protect-pdf": {
    ...
    items: [
      ...
      { q: "Mes fichiers sont-ils envoyés à un serveur ?", a: "Oui. Le chiffrement d'un PDF nécessite notre serveur de conversion sécurisé : chaque fichier est donc téléversé, chiffré, renvoyé, puis supprimé après le traitement — sans compte et sans conserver de copie après votre téléchargement. Le mot de passe est envoyé uniquement pour appliquer le chiffrement et n'est pas conservé." },
...
const FAQS_JA: Record<string, { title: string; items: Array<{ q: string; a: string }> }> = {
  ...
  "batch-protect-pdf": {
    ...
    items: [
      ...
      { q: "ファイルはサーバーにアップロードされますか？", a: "はい。PDFの暗号化には当社の安全な変換サーバーが必要なため、各ファイルはアップロードされ、暗号化されて返され、処理後に削除されます——アカウントは不要で、ダウンロード後にコピーは一切保持されません。パスワードは暗号化を設定するためにのみ送信され、保存されません。" },
      ...
      { q: "パスワードのルールやファイル数の制限はありますか？", a: "パスワードは英字・数字・アンダースコア（_）のみを使った4〜32文字でなければなりません——これによりあらゆるPDFリーダーで安全に適用できます。1バッチあたり最大30ファイルを暗号化でき、それ以上はツールをもう一度実行してください。厳密なサイズ上限はありませんが、各ファイルをサーバーにアップロードして暗号化するため、非常に大きなバッチは時間がかかります。" },
