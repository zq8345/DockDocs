...
function makeLabel(fmt: Fmt, n: number, total: number, zh: boolean, hant = false, ko = false): string {
  ...
  if (fmt === "page") return zh ? conv(`第 ${n} 页`) : ko ? `${n}페이지` : `Page ${n}`;
  if (fmt === "slash") return `${n} / ${total}`;
  if (fmt === "of") return zh ? conv(`${n} / 共 ${total}`) : ko ? `전체 ${total} 중 ${n}` : `${n} of ${total}`;
  return `${n}`;
}
...
export function PageNumbersClient({ locale = "en", embedded = false }: { locale?: Locale; embedded?: boolean }) {
  ...
  const previewLabel = useMemo(() => makeLabel(fmt, startAt, Math.max(1, numPages), zh, hant, ko), [fmt, startAt, numPages, zh, hant, ko]);
  ...
  const apply = useCallback(async () => {
    ...
    try {
      ...
      pages.forEach((page, i) => {
        ...
        const label = makeLabel(fmt, n, total, zh, hant, ko);
  ...
  }, [from, to, startAt, fmt, margin, pos, fileName, zh, hant, ko, t, locale, childLocale]);
  ...
  return (
    <div className={`${embedded ? "mx-auto w-full max-w-3xl px-8 pb-10 pt-4" : "mx-auto max-w-5xl px-5 pb-16 sm:px-6 sm:pb-20 pt-12 sm:pt-16"}`}>
      ...
      {!embedded && <ToolFaq tool="page-numbers" locale={locale} />}
    </div>
  );
}
