...
const STR = {
  ...
  zh: {
    ...
    note: "每份 PDF 都将需要此密码才能打开。已加密的 PDF 会被跳过。文件在我们的转换服务器上加密，处理后即删除。",
    err: "出错了：",
  },
  es: {
    title: "Cifrar por lotes",
    subtitle: "Define una sola contraseña y bloquea una carpeta entera de PDF: cada uno se cifra en nuestro servidor de conversión seguro y se empaqueta en un único ZIP para descargar.",
    ...
    note: "Cada PDF requerirá esta contraseña para abrirse. Los PDF que ya están cifrados se omiten. Los archivos se cifran en nuestro servidor de conversión y se eliminan tras el procesamiento.",
    err: "Algo salió mal: ",
  },
  pt: {
    title: "Criptografar em lote",
    subtitle: "Defina uma senha e bloqueie uma pasta inteira de PDFs: cada um é criptografado no nosso servidor de conversão seguro e empacotado em um único ZIP para download.",
    ...
    note: "Cada PDF exigirá esta senha para ser aberto. PDFs já criptografados são ignorados. Os arquivos são criptografados no nosso servidor de conversão e removidos após o processamento.",
    err: "Algo deu errado: ",
  },
  fr: {
    title: "Chiffrement en lot",
    subtitle: "Définissez un seul mot de passe et verrouillez un dossier entier de PDF : chacun est chiffré sur notre serveur de conversion sécurisé et regroupé dans un unique ZIP à télécharger.",
    ...
    note: "Chaque PDF nécessitera ce mot de passe pour être ouvert. Les PDF déjà chiffrés sont ignorés. Les fichiers sont chiffrés sur notre serveur de conversion et supprimés après le traitement.",
    err: "Une erreur est survenue : ",
  },
  ja: {
    title: "一括暗号化",
    subtitle: "パスワードを1つ設定して、フォルダ内のPDFをまとめてロック——各ファイルは当社の安全な変換サーバーで暗号化され、1つのZIPにまとめてダウンロードできます。",
...
const SECTIONS: Record<"en" | "zh" | "es" | "pt" | "fr" | "ja", ToolSectionsContent> = {
  en: {
    ...
    benefits: [
      { title: "One password, a whole folder", description: "Set a single password once and apply standard AES encryption to every PDF in the batch — no opening and re-saving files one at a time." },
  ...
  zh: {
    benefitsTitle: "为什么用 DockDocs 批量加密 PDF",
    benefitsDescription: "用一个密码锁住整个文件夹的 PDF，全部打包成一个 ZIP 取回。",
    benefits: [
      { title: "一个密码，整个文件夹", description: "只需设一次密码，就对批次里的每个 PDF 应用标准 AES 加密——不必逐个打开再另存。" },
  ...
  es: {
    benefitsTitle: "Por qué cifrar PDF por lotes con DockDocs",
    benefitsDescription: "Bloquea una carpeta entera de PDF con una sola contraseña y recupéralos todos en un único ZIP.",
    benefits: [
      { title: "Una contraseña, toda una carpeta", description: "Define una contraseña una sola vez y aplica cifrado AES estándar a cada PDF del lote, sin abrir y volver a guardar archivos uno por uno." },
  ...
  pt: {
    benefitsTitle: "Por que criptografar PDF em lote com o DockDocs",
    benefitsDescription: "Bloqueie uma pasta inteira de PDFs com uma só senha e receba todos de volta em um único ZIP.",
    benefits: [
      { title: "Uma senha, uma pasta inteira", description: "Defina uma senha uma única vez e aplique criptografia AES padrão a cada PDF do lote, sem abrir e salvar de novo arquivo por arquivo." },
  ...
  fr: {
    benefitsTitle: "Pourquoi chiffrer des PDF par lot avec DockDocs",
    ...
    benefits: [
      { title: "Un mot de passe, tout un dossier", description: "Définissez un mot de passe une seule fois et appliquez un chiffrement AES standard à chaque PDF du lot, sans ouvrir et réenregistrer les fichiers un par un." },
  ...
  ja: {
    benefitsTitle: "DockDocs で PDF を一括暗号化する理由",
    benefitsDescription: "1 つのパスワードでフォルダ内の PDF をまとめてロックし、1 つの ZIP で受け取れます。",
    benefits: [
      { title: "1 つのパスワードでフォルダ全体", description: "パスワードを一度設定するだけで、バッチ内のすべての PDF に標準的な AES 暗号化を適用——1 ファイルずつ開いて保存し直す必要はありません。" },
