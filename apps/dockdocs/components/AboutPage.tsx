...
const content = {
  en: {
    ...
    doHead: "We do",
    neverHead: "We never",
    doRows: ["Process files in your browser", "Delete cloud temp-files right after your download", "Label every cloud step up front", "Cite the source of an AI answer — or say when we can't", "Let you start with no account"],
    ...
    cloudSub: "A few tools (Office conversion, AI chat) need a server: labeled before you start, encrypted in transit, and deleted from the conversion service right after your download completes. When the AI answers a question or flags a finding, it shows the exact passage behind it — quoted from your file and verified to be there — and tells you when something can't be traced, instead of inventing a source.",
  ...
  zh: {
    ...
    doHead: "我们会",
    neverHead: "我们绝不",
    doRows: ["在你的浏览器里处理", "下载完成后即刻删除云端临时副本", "云端步骤事先标注", "标注 AI 答案的出处——无法溯源时就明说", "无需注册即可开始"],
    neverRows: ["存储你的文档", "用你的文件训练我们自己的模型", "出售或分享你的数据", "发邮件才能取消的套路", "隐藏的上传"],
    tableCaption: "图 0.4 — 完整的约定。",

    cloudEyebrow: "// 对云端坦诚",
    cloudHeading: "少数工具确实要用云端——它会明说。",
    cloudSub: "少数工具（Office 转换、AI 问答）需要服务器：开始前标注、传输加密、下载完成后即从转换服务中删除。AI 回答问题或标出某项发现时，会把支撑它的原文原句展示给你——从你的文件里逐字引用、并核实确实在文中——若有内容无法溯源，它会如实说明，而不是编造一个出处。",
  ...
  es: {
    ...
    doRows: ["Procesar archivos en tu navegador", "Borrar los temporales en la nube al finalizar tu descarga", "Señalar cada paso en la nube por adelantado", "Citar la fuente de una respuesta de IA — o avisar cuando no podemos", "Dejarte empezar sin crear cuenta"],
    ...
    cloudSub: "Algunas herramientas (conversión de Office, chat con IA) necesitan un servidor: avisado antes de empezar, cifrado en tránsito y borrado del servicio de conversión nada más finalizar tu descarga. Cuando la IA responde una pregunta o señala un hallazgo, te muestra el pasaje exacto que lo respalda —citado de tu archivo y comprobado que está realmente ahí— y te avisa cuando algo no se puede rastrear, en lugar de inventarse una fuente.",
  ...
  pt: {
    ...
    doRows: ["Processar arquivos no seu navegador", "Excluir temporários na nuvem assim que o seu download concluir", "Indicar cada etapa na nuvem com antecedência", "Citar a fonte de uma resposta de IA — ou avisar quando não conseguimos", "Deixar você começar sem criar conta"],
    ...
    cloudSub: "Algumas ferramentas (conversão do Office, chat com IA) precisam de um servidor: avisado antes de começar, criptografado em trânsito e excluído do serviço de conversão assim que o seu download for concluído. Quando a IA responde a uma pergunta ou aponta uma constatação, ela mostra o trecho exato que a fundamenta — citado do seu arquivo e confirmado que está mesmo lá — e avisa quando algo não pode ser rastreado, em vez de inventar uma fonte.",
  ...
  fr: {
    ...
    doRows: ["Traiter les fichiers dans votre navigateur", "Supprimer les temporaires cloud dès votre téléchargement terminé", "Signaler chaque étape cloud à l'avance", "Citer la source d'une réponse IA — ou le dire quand on ne peut pas", "Vous laisser commencer sans compte"],
    ...
    cloudSub: "Certains outils (conversion Office, chat IA) ont besoin d'un serveur : signalé avant de commencer, chiffré en transit et supprimé du service de conversion dès que votre téléchargement est terminé. Quand l'IA répond à une question ou relève un élément, elle montre le passage exact qui l'appuie — cité depuis votre fichier et vérifié qu'il s'y trouve — et vous indique quand quelque chose ne peut pas être tracé, au lieu d'inventer une source.",
  ...
  ja: {
    ...
    doHead: "やること",
    neverHead: "やらないこと",
    doRows: ["ファイルをブラウザ内で処理", "ダウンロード完了後にクラウドの一時ファイルを削除", "クラウド処理は事前に明示", "AI回答の根拠を提示——できない時はそう言う", "アカウントなしで開始できる"],
    ...
    cloudSub: "一部のツール(Office変換、AIチャット)はサーバーを必要とします——開始前に明示し、転送中は暗号化、ダウンロード完了後に変換サービスから削除します。AIが質問に答えたり指摘をする際は、その根拠となる原文箇所を——あなたのファイルから引用し実在を確認したうえで——示し、たどれない場合は出典を作らず正直に伝えます。",
  ...
  de: {
    ...
    doRows: ["Dateien in Ihrem Browser verarbeiten", "Temporäre Cloud-Dateien nach Ihrem Download löschen", "Jeden Cloud-Schritt vorab kennzeichnen", "Die Quelle einer KI-Antwort nennen — oder sagen, wenn wir es nicht können", "Sie ohne Konto starten lassen"],
  ...
  ko: {
    heroEyebrow: "// DockDocs 소개",
    heroPre: "당신의 문서는 ",
    heroAccent: "오직 당신의 것입니다.",
    heroSub: "대부분의 DockDocs 도구는 전적으로 브라우저 안에서 실행됩니다 — 파일은 당신을 떠나지 않기에 저희에게 닿지 않습니다. 로컬 처리가 기본값이며, 따로 켜야 하는 설정이 아닙니다.",
    heroCaption: "그림 0.1 — 당신의 파일, 그리고 그것이 머무는 곳.",

    flowEyebrow: "// 파일을 따라가 보세요",
    flowHeading: "당신의 파일이 실제로 가는 곳.",
    flowSub: "대부분의 도구에서 처리는 당신의 기기에서 이루어집니다 — 파일은 이 선을 결코 넘지 않습니다.",
    flowDevice: "당신의 기기",
    flowNet: "인터넷",
    flowServers: "그들의 서버",
    flowZero: "0바이트 업로드",
    flowLeaves: "당신의 기기를 떠납니다",
    flowNoCross: "이 선을 넘는 것은 없습니다",
    flowWouldGo: "원래라면 파일이 가게 될 곳",
    flowStays: "당신의 기기에 머뭅니다",
    flowCaption: "그림 0.2 — 대부분의 도구에서 당신의 파일은 경계를 결코 넘지 않습니다.",

    proofEyebrow: "// 말만 믿지 마세요",
    proofHeading: "저희를 믿지 마세요. F12를 누르세요.",
    proofSub: "브라우저의 네트워크 탭을 열고 브라우저 내 도구를 실행해 보세요 — 업로드가 전혀 없는 것을 보게 됩니다. Wi-Fi를 꺼도 그대로 작동합니다.",
    proofZero: "0개 요청 · 0B 전송됨",
    proofOffline: "Wi-Fi를 꺼도 작동합니다.",
    proofCallout: "이것이 증명의 전부입니다",
    proofCaption: "그림 0.3 — 실시간으로 포착한 실제 브라우저 내 도구. 아무것도 나가지 않습니다.",

    tableEyebrow: "// 솔직하게 말하면",
    tableHeading: "저희가 하는 일. 저희가 결코 하지 않는 일.",
    tableSub: "숨은 약관은 없습니다. 왼쪽 목록이 제품의 전부이고, 오른쪽 목록은 저희가 하기를 거부하는 모든 것입니다.",
    doHead: "저희가 하는 일",
    neverHead: "저희가 결코 하지 않는 일",
    doRows: ["브라우저에서 파일을 처리합니다", "다운로드 완료 직후 클라우드 임시 파일을 삭제합니다", "모든 클라우드 단계를 미리 표시합니다", "AI 답변의 출처를 표시합니다 — 찾을 수 없을 때는 그렇다고 알립니다", "계정 없이 시작할 수 있게 합니다"],
    neverRows: ["당신의 문서를 저장하지 않습니다", "당신의 파일로 저희 모델을 학습시키지 않습니다", "당신의 데이터를 팔거나 공유하지 않습니다", "이메일로만 해지되는 함정", "숨겨진 업로드"],
    tableCaption: "그림 0.4 — 약속의 전부.",

    cloudEyebrow: "// 클라우드에 대해 솔직하게",
    cloudHeading: "도구가 클라우드를 사용할 때는, 그렇다고 알립니다.",
    cloudSub: "일부 도구(Office 변환, AI 채팅)는 서버가 필요합니다: 시작 전에 표시되고, 전송 중 암호화되며, 다운로드가 완료된 직후 변환 서비스에서 삭제됩니다. AI가 질문에 답하거나 발견한 점을 표시할 때는, 그 근거가 되는 정확한 구절을 보여 줍니다 — 당신의 파일에서 그대로 인용하고 실제로 거기에 있음을 확인합니다 — 그리고 출처를 만들어 내는 대신, 추적할 수 없을 때는 그렇다고 알립니다.",
    cloudSteps: ["사전 표시", "전송 중 암호화", "처리", "복사본 삭제"],
    cloudNotKept: "보관하지 않음",
    aiSummary: "AI 답변",
    aiCite: "출처",
    cloudCaption: "그림 0.5 — 클라우드 단계는 표시되고 삭제됩니다; AI 답변은 추적 가능합니다.",

    statsEyebrow: "// 중요한 숫자",
    statsHeading: "중요한 숫자는 0입니다.",
    statsSub: "독립적으로 구축되었습니다 — 투자자가 아니라 사용하는 사람들이 후원합니다. 데이터를 긁어모으거나 어떤 대가를 치르더라도 성장을 좇으라는 압박이 없습니다.",
    stats: [
      { n: "0", l: "브라우저 내 도구에서 저장되는 파일", z: true },
      { n: "0", l: "저희 모델 학습에 사용되는 파일", z: true },
      { n: "$0", l: "시작 비용 — 계정 불필요", z: true },
      { n: "~50", l: "PDF 및 AI 도구", z: false },
    ],
    relPoints: [
      { t: "결제", b: "라이선스를 보유한 등록 판매자(Merchant of Record) Creem이 처리합니다 — 저희는 당신의 카드를 보지 않습니다." },
      { t: "환불", b: "7일 환불 보장, 언제든 해지 가능." },
      { t: "종속 없음", b: "당신의 데이터는 당신의 것입니다; 언제든 내보낼 수 있습니다." },
      { t: "견고함", b: "브라우저 내 도구는 언제나 당신의 기기에서 실행됐습니다 — 결코 인질로 잡히지 않습니다." },
    ],
    statsCaption: "그림 0.6 — 저희가 무엇을 저장하고, 무엇을 청구하며, 누구에게 책임지는지.",

    ctaEyebrow: "// 사용해 보세요",
    ctaHeading: "도구를 사용해 보세요. 업로드 없음. 계정 없음.",
    ctaSub: "아무 도구나 열고 네트워크 탭을 지켜보세요 — 브라우저 내 도구는 아무것도 나가지 않습니다.",
    cta1: "도구 둘러보기", cta2: "프라이버시 작동 방식 보기",
  },
} as const;
...
export function AboutPage({ locale = "en" }: { locale?: Locale }) {
  ...
  // ko has a content block (AI translation, pending native review); any other
  // locale without a block falls back to English via `?? content.en`.
