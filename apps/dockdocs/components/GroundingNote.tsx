...
const BODY: Record<Variant, Record<string, string>> = {
  chat: {
    ...
    ja: "どの回答も、あなたがアップロードした文書の本文から生成されます。モデルの一般知識からではありません。DockDocs は各回答の根拠となった具体的な箇所を示すので、どの主張もその出典までたどれます。文書に十分な根拠がない場合、DockDocs は推測せずにそのまま「根拠が足りない」と伝え、あなたのファイル内で見つけられない引用は、事実として提示するのではなく「検証できない」と明示します。この「原文への根拠づけ」こそが、信頼できる文書アシスタントを一般的なチャットボットと分けるものです。",
  },
  contract: {
