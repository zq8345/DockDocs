...
export function LanguageSwitcher() {
  ...
  return (
    <div className="relative group">
      ...
      {/* Invisible bridge keeps dropdown open when moving mouse */}
      <div className="absolute right-0 top-full z-50 hidden w-[360px] group-hover:block pt-2">
        <div className="rounded-[var(--radius)] border border-[color:var(--line)] bg-[color:var(--surface)] p-3 shadow-[0_16px_48px_rgba(0,0,0,0.4)]">
          <div className="grid gap-1" style={{ gridTemplateColumns: `repeat(${cols}, 1fr)` }}>
            {languageOptions.map((option) => {
              const isActive = option.locale === activeLocale;
              return (
                <button key={option.locale} type="button" onClick={() => switchTo(option.locale)}
                  className={`flex items-center gap-1.5 rounded-[var(--radius-sm)] px-2.5 py-2 text-left text-[13px] transition ${
                  ...
                  }`}><span>{option.native}</span></button>
              );
            })}
          </div>
        </div>
