...
// Locks the en/zh/es/pt/fr/ja mapping + the English fallback for unsupported/junk input.
...
test("resolveAnswerLocale passes through each supported locale", () => {
  for (const loc of ["en", "zh", "es", "pt", "fr", "ja"] as const) {
...
test("resolve + name compose: a junk locale answers in English, ja answers in Japanese", () => {
  assert.equal(answerLanguageName(resolveAnswerLocale("de")), "English");
